...
const APP = path.join(root, 'insurance_core')
const PKG = path.join(APP, 'insurance_core')
const MOD = path.join(PKG, 'insurance_core')
...
fixtures = [
	...
	{"dt": "Custom Field", "filters": [["module", "=", "Insurance Core"]]},
]

website_route_rules = [
	{"from_route": "/insurance_core/<path:app_path>", "to_route": "insurance_core"},
	{"from_route": "/frontend/<path:app_path>", "to_route": "insurance_core"},
]
...
write(path.join(PKG, 'modules.txt'), 'Insurance Core\n')
write(path.join(PKG, 'patches.txt'), '[pre_model_sync]\n\n[post_model_sync]\ninsurance_core.patches.v1_0.rename_module_to_insurance_core\n')
...
write(path.join(PKG, 'config', 'docs.py'), 'source_link = "https://github.com/VivaswanWorks/insurance_core"\n')
...
write(
  path.join(MOD, 'workspace', 'insurance_core', 'insurance_core.json'),
  JSON.stringify(
    {
      charts: [],
      content: '[]',
...
write(path.join(MOD, 'workspace', 'insurance_core', '__init__.py'), '')
